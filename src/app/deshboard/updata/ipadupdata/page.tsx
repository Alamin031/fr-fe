...
export default function ProductsTable() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URI}/ipadlist/all`);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URI}/ipadlist/delete/${id}`);
  ...
  const openEditDialog = async (productLinkName: string) => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BASE_URI}/ipadlist/getBySlug/${productLinkName}`
      );
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_BASE_URI}/ipadlist/update/${editingProduct.productlinkname}`,
        editingProduct
      );
