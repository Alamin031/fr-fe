...
const Page = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await fetch(`/api/getproduct/ipaddetails/product?name=${productName}`);
        const data = await response.json();
        setProduct(data);
        
        // Set default selections
        if (data.storageConfigs?.length > 0) {
          setSelectedStorage(data.storageConfigs[0]);
        }
        if (data.colorImageConfigs?.length > 0) {
          setSelectedColor(data.colorImageConfigs[0]);
          setCurrentImage(data.colorImageConfigs[0].image);
        }
        if (data.simConfigs?.length > 0) {
          setSelectedSim(data.simConfigs[0]);
        }
        if (data.dynamicRegions?.length > 0) {
  ...
  useEffect(() => {
    if (product) {
      ...
      const regionPrice = parseFloat(selectedRegion?.price || 0);
            ...
            {product.colorImageConfigs?.map((config) => (
              <button
                ...
                className={`aspect-square rounded-lg overflow-hidden border-2 transition-all ${
                  selectedColor?.id === config.id ? 'border-blue-600' : 'border-gray-200'
                }`}
              >
                <img 
                  src={config.image} 
  ...
  <span>
    {selectedStorage?.shortDetails && (
      <span className="ml-1">{selectedStorage.shortDetails}</span>
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-700 mb-3">Connectivity</h3>
          ...
          <div className="">
            ...
            <div className="flex gap-3">
              <button 
                onClick={() => {
                  const message = `Hi, I'm interested in ${product.name} ${selectedStorage?.label || ''} - ${selectedColor?.color || ''} for ৳${totalPrice.toLocaleString()}`;
              ...
              >
                <svg className="w-5 h-5 max-sm:w-3 max-sm:h-3" fill="currentColor" viewBox="0 0 24 24">
          ...
          {/* {selectedStorage?.shortDetails && (
            <div className="bg-gray-50 p-4 rounded-lg">
              <p className="text-sm text-gray-600">{selectedStorage.shortDetails}</p>
            </div>
          )} */}
        </div>
      </div>
      ...
      <div className="mt-16">
        <h2 className="text-2xl max-sm:text-[15px] font-bold text-gray-900 mb-6">Technical Specifications</h2>
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
          <div className="divide-y divide-gray-200">
            {product.details?.map((detail, idx) => (
              <div key={detail.id} className={`grid grid-cols-3 gap-4 p-4 ${idx % 2 === 0 ? 'bg-gray-50' : ''}`}>
                <div className="font-semibold text-[13px] text-gray-700">{detail.label}</div>
                <div className="col-span-2 text-[11px] text-gray-600">{detail.value}</div>
              </div>
            ))}
          </div>
        </div>
